...
class OpenCloseRangeFn(beam.DoFn):
  def process(self, element):
    ...
    Volume_ETH = record.get('Volume_ETH')
...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    query_results = p | 'Read from BigQuery' >> beam.io.Read(beam.io.BigQuerySource(
        query = 'SELECT * FROM crypto_market_data_transformed.ETH_1H_Split LIMIT 1000')
        )
    ...
    # write PCollection to log file
    range_pcoll | 'Write to log 1' >> WriteToText('ETH_ID_range.txt')
    ...
    qualified_table_name = PROJECT_ID + ':crypto_market_data_transformed.ETH_ID_range'
