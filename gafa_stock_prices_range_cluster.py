...
with beam.Pipeline('DataflowRunner', options=opts) as p:
    ...
    query_results = p | 'Read from BigQuery' >> beam.io.Read(beam.io.BigQuerySource(
        query = 'SELECT * FROM econ_data.gafa_stock_prices')
        )
    ...
    # write PCollection to log file
    range_pcoll | 'Write to log 1' >> WriteToText('Gafa.txt')
    ...
    qualified_table_name = PROJECT_ID + ':econ_data.gafa_stock_prices_range'
